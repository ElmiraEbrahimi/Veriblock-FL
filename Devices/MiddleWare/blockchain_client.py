...
from devices.utils.gas import log_receipt
...
class BlockChainConnection:
    ...
    def init_contract(self, accountNR):
        if self.is_connected() and accountNR == 0:
            ...
            self.__await_Trainsaction(thxHash, accountNR, 'functions.initModel')
            thxHash = self.FLcontractDeployed.functions.map_temp_to_global().transact(
                {"from": self.web3Connection.eth.accounts[accountNR]}
            )
            self.__await_Trainsaction(thxHash, accountNR, 'functions.map_temp_to_global')
            ...
            self.__await_Trainsaction(thxHash, accountNR, 'functions.updateVerifier')
    ...
    def __await_Trainsaction(self, thxHash, accountNr, desc: str):
    ...
    def roundUpdateOutstanding(self, accountNR):
        ...
        if not newround:
            try:
                ...
                self.__await_Trainsaction(txhash, accountNR, 'functions.end_update_round')
            except Exception:
    ...
    def __update_with_proof(self, weights, bias, accountNR, proof):
        ...
        weights = [[int(x) for x in y] for y in weights]
        bias = [int(x) for x in bias]
        thxHash = self.FLcontractDeployed.functions.update_with_proof(
            weights, bias, a, b, c, inputs
        ).transact({"from": self.web3Connection.eth.accounts[accountNR]})
        self.__await_Trainsaction(thxHash, accountNR, 'functions.update_with_proof')
    ...
    def __update_without_proof(self, weights, bias, accountNR):
        ...
        self.__await_Trainsaction(thxHash, accountNR, 'functions.update_without_proof')
        print(f"AccountNr = {accountNR}: UPDATE SUCCESSFUL")
